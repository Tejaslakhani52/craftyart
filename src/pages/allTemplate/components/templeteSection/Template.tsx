...
export default function Templete({ setTemplatesData }: any) {
  ...
  const fetchData = async (pages: number) => {
    const templates_ = await api.getCategoryDatas({
      debug_key: "debug",
      cat_id:
        categoryId === "invitation" ? "a4-invitation" : (categoryId as any),
      limit: 50,
      page: pages,
    });
    setIsloading(true);
    ...
    if (categoryId === "latest") {
      setTemplatesData(data?.latestMeta);
    } else if (categoryId === "trending") {
      setTemplatesData(data?.trendingData);
    } else if (categoryId === "invitation") {
      setTemplatesData(data?.invitationData);
    } else setTemplatesData(templates_);
